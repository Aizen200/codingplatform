import React from 'react'
import { Filter, User } from 'lucide-react'

const Topics = () => {
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-slate-200 px-6 py-4">

   
      <div className="flex items-center justify-between mb-8">
        <button className="text-2xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
          Algorythm
        </button>

        <div className="flex items-center gap-3">
          <input
            type="text"
            placeholder="Search problems"
            className="bg-slate-800/60 border border-slate-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-purple-500"
          />
          <button className="bg-gradient-to-r from-purple-500 to-pink-500 px-4 py-2 rounded-lg text-sm font-medium">
            <Filter size={16} />
          </button>
        </div>

        <button className="w-9 h-9 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 flex items-center justify-center">
          <User size={18} />
        </button>
      </div>


      <div className="flex gap-8">

        <div className="w-80 h-72 bg-slate-900/60 border border-slate-800 rounded-2xl p-6 backdrop-blur-md">
          <h2 className="text-xl font-semibold mb-4 text-purple-400">
            Topic Name
          </h2>

          <div className="space-y-2 text-sm">
            <p className="text-green-400">Easy: 5 questions</p>
            <p className="text-yellow-400">Medium: 5 questions</p>
            <p className="text-red-400">Hard: 5 questions</p>
          </div>
        </div>

        <div className="flex-1 bg-slate-900/60 border border-slate-800 rounded-2xl overflow-hidden">
          <table className="w-full">
            <tbody>
              <tr className="border-b border-slate-800 hover:bg-slate-800/40 transition">
                <td className="px-6 py-4">Question of that given topic</td>
              </tr>
              <tr className="border-b border-slate-800 hover:bg-slate-800/40 transition">
                <td className="px-6 py-4">Another question</td>
              </tr>
              <tr className="hover:bg-slate-800/40 transition">
                <td className="px-6 py-4">Another question</td>
              </tr>
            </tbody>
          </table>
        </div>

      </div>
    </div>
  )
}

export default Topics
